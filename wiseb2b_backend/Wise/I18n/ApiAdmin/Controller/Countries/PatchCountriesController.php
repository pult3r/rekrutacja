...
use Wise\I18n\ApiAdmin\Dto\Countries\PutCountriesDto;
use Wise\I18n\ApiAdmin\Service\Countries\Interfaces\PutCountriesServiceInterface;

class PatchCountriesController extends AdminApiBaseController
{
    protected array $requiredApiScopes = [
        ScopeNames::GENERAL_ACCESS,
        ScopeNames::GENERAL_PATCH,
        ScopeNames::COUNTRIES_ALL,
        ScopeNames::COUNTRIES_PATCH,
    ];

    public function __construct(
        private readonly PutCountriesServiceInterface $service
    ) {}
    ...
    #[OA\Patch(
        description: 'Modyfikacja listy krajów',
        tags: ['Countries']
    )]
    #[OA\RequestBody(
        required: true,
        content: new OA\JsonContent(ref: "#/components/schemas/PutCountriesDto", type: "object")
    )]
    #[OA\Parameter(
        name: 'x-request-uuid',
    ...
    public function patchCountriesAction(Request $request): JsonResponse
    {
        return $this->service->process($request->headers->all(), $request->getContent(), PutCountriesDto::class, true);
    }
}
