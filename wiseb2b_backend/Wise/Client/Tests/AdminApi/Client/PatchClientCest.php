...
use Wise\Order\Domain\Order\OrderRepositoryInterface;
use Wise\Order\Repository\OrderHistoryRepositoryInterface;
...
class PatchClientCest
{
    ...
    private ?OrderHistoryRepositoryInterface $orderHistoryRepository = null;
     ...
     * 1. Dodanie klienta z poprawnymi danymi przez Patch
     * 2. Weryfikacja czy klienta został poprawnie dodany przez GET oraz Repository
     */
    public function correctPatchClient(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawnie dodanie nowego klienta. Weryfikacja za pomoca bazy danych i repozytorium ');

        //Patch
        $clientId = $I->generateId('CLIENT');
        $object = $this->getClientObjectArray($clientId, $I);
        $responsePatch = $I->sendPatchAsJson('/clients', [
            'objects' => [
                $object
            ]
        ]);
        ...
        $I->seeObjectWithElementsInResponse($responsePatch, $clientId, [
            'id' => $clientId,
            'message' => 'SUCCESS',
            'status' => 1
        ]);
     ...
     */
    #[Skip('WIS-2173')]
    public function correctPatchClientModificationAddress(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja adresu klienta');
        ...
        $I->seeResponseCodeIs(HttpCode::OK);
        $I->seeResponseContainsJson(['status' => 1, 'message' => 'SUCCESS']);
        $I->seeObjectWithElementsInResponse($responsePatch, $client->getIdExternal(), [
...
//        $I->assertEquals($objectModification['register_address']['name'], $client->getRegisterAddress()->getName());
     ...
     */
    public function correctPatchClientModificationPhoneNumber(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja numeru telefonu');
     ...
     */
    public function correctPatchClientModificationEmail(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja adresu email');
     ...
     */
    #[Skip('WIS-2173')]
    public function correctPatchClientModificationAccountBank(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja konta bankowego');
     ...
     */
    public function correctPatchClientModificationDefaultPaymentMethod(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja domyślnej metody płatności');
     ...
     */
    public function correctPatchClientModificationDefaultDeliveryMethod(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawna modyfikacja domyślnej metody dostawy');
    ...
    /**
     * Weryfikacja możliwości nadpisania metod płatności przez Patch
     * 1. Dodanie klienta
     * 2. Zmiana domyślnej metody dostawy
     * 3. Pobranie klienta i weryfikacja domyślnej metody dostawy
     */
    #[Skip('Oczekujemy na odpowiedź Wojtka => Wojciech Struski - zagadnienia do omówienia')]
    public function correctPatchClientModificationOverwritingPayments(AdminApiTester $I)
    {
        $I->wantToTest('PATCH /api/admin/Client - Poprawne nadpisanie metod płatności');
        ...
        /**
         * Problem jest taki, że w pliku PatchClientService.php w metodzie prepareAndPatchClientPaymentMethod (linia 108)
    ...
    private function prepareDataForTests(AdminApiTester $I): void
    {
        ...
        $this->orderRepository = $I->grabService(OrderRepositoryInterface::class);
        $this->orderHistoryRepository = $I->grabService(OrderHistoryRepositoryInterface::class);
